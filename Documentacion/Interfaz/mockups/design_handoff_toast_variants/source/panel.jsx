...
// ----- Consola MSS · feed de reconocimiento de pantallas (FSM en vivo) ----------
const MSS_COLORS = {
  info: "var(--info)", purple: "var(--purple)", yellow: "var(--yellow)",
  positive: "var(--positive)", warning: "var(--warning)", muted: "var(--text-muted)",
};

const MSS_SEQ = [
  { tag: "NAV",   c: "info",     msg: "transici\u00f3n detectada \u00b7 main_menu \u2192 agents" },
  { tag: "MSS",   c: "purple",   msg: "STATE  MAIN_MENU \u25b8 AGENTS" },
  { tag: "OCR",   c: "yellow",   msg: "pantalla reconocida \u00b7 AGENTS_LIST   conf 0.98" },
  { tag: "CAP",   c: "muted",    msg: "frame  \u0394 22.6%  \u2192 dispara OCR" },
  { tag: "NAV",   c: "info",     msg: "transici\u00f3n detectada \u00b7 agents \u2192 drive_discs" },
  { tag: "MSS",   c: "purple",   msg: "STATE  AGENTS \u25b8 DRIVE_DISCS" },
  { tag: "OCR",   c: "yellow",   msg: "pantalla reconocida \u00b7 DISC_DRIVE_DETAIL  conf 0.96" },
  { tag: "CAP",   c: "muted",    msg: "frame  \u0394 18.4%  ROI estable \u00b7 lock" },
  { tag: "OCR",   c: "positive", msg: "disco parseado \u00b7 #00482  set=Jazz Ca\u00f3tico  slot 4" },
  { tag: "SCORE", c: "positive", msg: "eval \u2192 Yanagi  87.3  \u2713 recomendar EQUIPAR" },
  { tag: "MSS",   c: "warning",  msg: "WAIT  esperando acci\u00f3n del usuario\u2026" },
];

function mssStamp(i) {
  const total = (23 * 3600 + 41 * 60) + i * 0.27;
  const s = Math.floor(total);
  const ms = Math.floor((total - s) * 1000);
  const p = (n, l = 2) => String(n).padStart(l, "0");
  return `${p(Math.floor(s / 3600) % 24)}:${p(Math.floor(s / 60) % 60)}:${p(s % 60)}.${p(ms, 3)}`;
}

function MssConsole() {
  const [lines, setLines] = React.useState(() =>
    MSS_SEQ.slice(0, 7).map((l, i) => ({ ...l, id: i, t: mssStamp(i) }))
  );
  const scrollRef = React.useRef(null);
  const idx = React.useRef(7);

  React.useEffect(() => {
    const iv = setInterval(() => {
      setLines((prev) => {
        const next = MSS_SEQ[idx.current % MSS_SEQ.length];
        idx.current += 1;
        const arr = [...prev, { ...next, id: idx.current, t: mssStamp(idx.current) }];
        return arr.slice(-24);
      });
    }, 1100);
  ...
  return (
    <PChamferBox
      ...
      pattern="carbon"
      style={{ height: 172, flexShrink: 0 }}
      innerStyle={{ height: 172, display: "flex", flexDirection: "column" }}
    >
      {/* Header */}
      <div style={{
        display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "7px 14px", borderBottom: "1px solid var(--border-subtle)",
        background: "rgba(0,0,0,0.4)", position: "relative", overflow: "hidden", flexShrink: 0,
      }}>
        <div style={{
          position: "absolute", top: 0, left: 0, width: "35%", height: "100%",
          background: "linear-gradient(90deg, transparent, rgba(157,78,221,0.12), transparent)",
          animation: "zzz-scan 3.6s linear infinite", pointerEvents: "none",
        }}/>
        <div style={{ display: "flex", alignItems: "center", gap: 10, zIndex: 1 }}>
          <span style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--purple)", boxShadow: "0 0 8px var(--purple)", animation: "zzz-blink 1.4s infinite" }}/>
          <span className="caps" style={{ fontSize: 11, color: "var(--text-primary)", letterSpacing: "0.14em", fontWeight: 600 }}>
            Consola MSS <span style={{ color: "var(--text-muted)" }}>· reconocimiento de pantallas</span>
          </span>
          <span className="num" style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--purple)", border: "1px solid var(--purple)", padding: "0 5px", borderRadius: 3, letterSpacing: "0.06em" }}>FSM v0.9.4</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.06em", zIndex: 1 }}>
          {["MAIN_MENU", "AGENTS", "DRIVE_DISCS"].map((s, i, a) => (
            <React.Fragment key={s}>
              <span style={{ color: i === a.length - 1 ? "var(--yellow)" : "var(--text-muted)", fontWeight: i === a.length - 1 ? 700 : 400 }}>{s}</span>
              {i < a.length - 1 && <span style={{ color: "var(--text-dim)" }}>›</span>}
            </React.Fragment>
          ))}
          <span style={{ color: "var(--text-dim)", margin: "0 4px" }}>·</span>
          <span className="caps" style={{ color: "var(--positive)", fontSize: 9, letterSpacing: "0.1em" }}>SYNC</span>
        </div>
      </div>

      {/* Body: estado actual + feed */}
      <div style={{ flex: 1, minHeight: 0, display: "flex" }}>
        {/* Estado actual de la FSM */}
        <div style={{
          width: 232, flexShrink: 0, borderRight: "1px solid var(--border-subtle)",
          padding: "9px 14px", display: "flex", flexDirection: "column", justifyContent: "center",
          gap: 7, background: "rgba(0,0,0,0.25)",
        }}>
          <div className="caps" style={{ fontSize: 9, color: "var(--text-muted)", letterSpacing: "0.14em" }}>ESTADO ACTUAL</div>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <span className="display caps" style={{ fontSize: 19, color: "var(--yellow)", fontWeight: 700, letterSpacing: "0.03em", lineHeight: 1 }}>DRIVE_DISCS</span>
            <span style={{ width: 7, height: 15, background: "var(--yellow)", animation: "zzz-blink 1s infinite" }}/>
          </div>
          <div style={{ fontSize: 10, color: "var(--text-secondary)" }}>subpantalla · <span style={{ color: "var(--text-primary)", fontWeight: 600 }}>DETAIL</span></div>
          <div style={{ display: "flex", gap: 16, marginTop: 3 }}>
            <div>
              <div className="caps" style={{ fontSize: 8, color: "var(--text-muted)", letterSpacing: "0.1em" }}>CONF OCR</div>
              <div className="num display" style={{ fontSize: 15, color: "var(--positive)", fontWeight: 700, lineHeight: 1.2 }}>0.96</div>
            </div>
            <div>
              <div className="caps" style={{ fontSize: 8, color: "var(--text-muted)", letterSpacing: "0.1em" }}>FPS</div>
              <div className="num display" style={{ fontSize: 15, color: "var(--info)", fontWeight: 700, lineHeight: 1.2 }}>18</div>
            </div>
            <div>
              <div className="caps" style={{ fontSize: 8, color: "var(--text-muted)", letterSpacing: "0.1em" }}>TRANS</div>
              <div className="num display" style={{ fontSize: 15, color: "var(--text-primary)", fontWeight: 700, lineHeight: 1.2 }}>3</div>
            </div>
          </div>
        </div>
        ...
        <div ref={scrollRef} style={{
          flex: 1, minWidth: 0, overflow: "auto", padding: "6px 0",
          fontFamily: "var(--font-mono)", fontSize: 11, lineHeight: 1.75,
        }}>
          {lines.map((l) => (
            <div key={l.id} style={{ display: "flex", gap: 10, padding: "0 14px", whiteSpace: "nowrap" }}>
              <span style={{ color: "var(--text-dim)" }}>{l.t}</span>
              <span style={{ color: MSS_COLORS[l.c], width: 52, flexShrink: 0, fontWeight: 600 }}>[{l.tag}]</span>
              <span style={{ color: l.c === "muted" ? "var(--text-muted)" : "var(--text-secondary)" }}>{l.msg}</span>
            </div>
          ))}
        </div>
      </div>
...
function DiscPanel() {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "16px 18px", display: "flex", flexDirection: "column", gap: 12, flex: 1, minHeight: 0, overflow: "auto" }}>
        {/* Set title + rarity */}
