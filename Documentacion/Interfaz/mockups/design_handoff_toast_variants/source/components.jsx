...
function Icon({ name, size = 14, color = "currentColor", strokeWidth = 1.6 }) {
  ...
  switch (name) {
    ...
    case "swap":
      return (<svg {...props}><path d="M6 8 H19"/><path d="M15 4 L19 8 L15 12"/><path d="M18 16 H5"/><path d="M9 12 L5 16 L9 20"/></svg>);
    default:
      return null;
  }
}
...
function BlockBox({
  ...
  const glowBox = glow === "yellow" ? "0 0 22px rgba(255,203,5,0.45)"
    ...
    : glow === "purple" ? "0 0 22px rgba(157,78,221,0.45)"
    : null;
...
function Rarity({ tier = "S", size = 24 }) {
  ...
  return (
    <div style={{
      ...
      <span className="display num" style={{ color: "#0a0a0a", fontSize: size * 0.58, fontWeight: 800, lineHeight: 1, textShadow: "0 1px 0 rgba(255,255,255,0.3)" }}>{tier}</span>
    </div>
  );
}
