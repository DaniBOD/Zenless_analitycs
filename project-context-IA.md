...
|------|-------|------:|-------|
| 1 Catálogos | `agents` | **52** | 45 originales + Cissia v2.7 + Billy Estelar v2.x + Velina v3.x + **Pyrois v3.x** (2026-06-21, **rango ∞ "infinito" + facción Faetón nuevos**, Éter/Ataque/M0, onboarding PARCIAL: identidad+stats Nv1 provisorios+synergy+score_thr+awakening; pendiente agent_thresholds Prydwen + splash + IA) + **Remielle Dan v3.1** (2026-07-28, mig 15, **elemento Lumen + facción Covenant of Dayat nuevos**, S/Anomalía/M0, onboarding PARCIAL igual que Pyrois pero con stats Nv1 **sin discos** = base puros; pendiente agent_thresholds + splash + avatar_ref + IA) + **Aria** (2026-08-16, mig 19, S/Éter/Anomalía/`Angels of Delusion`, Nv40 M0 — **el primero SIN vocabulario nuevo**: elemento, rol, facción y rango ya existían, cero CHECKs que tocar. Stats **efectivos** con discos+arma, cruzados contra `parse_agent_stats` (conf 0.973). Badges cosechados en las **3 superficies** el mismo día; pendiente agent_thresholds + IA) + **Claret Flint v3.2** (2026-09-17, mig 35, id 52, S/Eléctrico/**rol nuevo `Armero`**/**facción nueva `Flint Workshop`**, Nv60 M0 — columnas nuevas `dano_laceracion`/`acumulacion_afiladura`, `ataque` NULL (la ficha del Armero no lo muestra), arquetipo propio `ARMORER_DEF`, thresholds y substat prefs de Prydwen; pendiente badges en vivo, W-Engines e IA) |
| 1 | `weapons` | **60** | **−1 el 2026-09-13** (mig `_34`, `audit/migracion_34_weapons_unifica_severed_innocence_20260913.md`): *Inocencia sacrificada* estaba dos veces — la fila 22 *Serenidad cortada* del catálogo inicial era la misma arma (Severed Innocence); unificada en la 22 con el nombre de pantalla. Saneada 2026-07-27/28 (migraciones `_10`, `_11`, `_13` + `audit/weapons_catalog_20260728.md`): el catálogo se había cargado emparejando ES↔EN **por parecido**, así que rareza/tipo/`atk_base` venían heredados de otra arma. Corregido contra las 40 capturas del juego (**ATK a Nivel 60/60 discrimina la rareza: S ∈ {684,713,743}, A ∈ {594,624}**). +8 altas, −2 duplicados fusionados. Estado: **53 verificadas + 5 con `nombre_en IS NULL`** (laguna declarada, se cierra capturando). Faltan 42 de la lista canónica: no se pueden agregar offline, `nombre` es el nombre ESPAÑOL in-game y no hay fuente. Ya tiene `pasiva_modelada` + `sensibilidad_contexto`. **+2 el 2026-09-10** (mig `_27`, `audit/weapons_catalog_20260910.md`): *Inocencia sacrificada* y *Tetera esmeraldina*, los únicos huecos reales de los 6 que listó el censo — los otros 4 ya estaban y el OCR no los matcheaba (texto del arte pegado al nombre) |
| 1 | `disc_sets` | **30** | Auditada 2026-07-30 contra la pantalla "Preferencia de disco" (mig `_18`): se pasaron los **30 nombres del juego** por `DiscSetRepo.resolve_id` y **6 no resolvían** ⇒ `sync_equip` descartaba esos discos en silencio (20 % del catálogo). +2 altas del 3.1 (**Hado emplumado**, **Rosa espinosa**, con `nombre_en` NULL: la wiki no los publicó y `nombre_en` resuelve el logo) + **4 nombres españoles corregidos**, verificados **por el logo**: `Notas encadenadas`→Diario de una prisionera, `Aria brillante`→Aria radiante, `Soul Rock`→Rock espiritual, `Polar Metal`→Metal polar (los dos últimos tenían el **inglés en la columna española**, mismo defecto que ids 18/30 de `weapons`). `nombre_en` intacto ⇒ los íconos siguen. Ahora **30/30 resuelven, sin colisiones**. Antes: +Firmamento llameante (id 53) 2026-06-27; +Salón huracanado (id 52) 2026-06-26 |
| 1 | `agent_awakenings` | **15** | Niveles **confirmados in-game por DaniBOD 2026-07-29** (migs 16+17), sin ningún `nivel` en NULL. **6/6 (7):** Burnice, Ellen, Grace, Jane, Lycaon, N.°0:Anby, Rina. **1/6 (1):** N.°11 ⚠️ parcial. **0/6 (8):** Aria, Billy Estelar, Cissia, Harumasa, Nekomata, Pyrois, Remielle Dan, Velina. Invariante: `activo=1` ⟺ `nivel>0`. **Pero solo 1 fila tiene texto de efecto real** (Burnice, `stat_boost`): las otras 7 con nivel>0 son `pending_capture` — la tabla inventaria la deuda, todavía no alimenta scoring. El sistema no tiene captura implementada (cero refs a awakening/silueta en `app/core`): carga manual desde screenshots de la tienda Silueta Potencial ("Agotado"=nv6, "Límite ×N"=parcial). Al capturar N.°11 anotar **a qué nivel** corresponde el texto |
...
| 2 | `inventory_discs` | **367** | 292 equipados + 75 sueltos. ⚠️ Esta fila decía **334** hasta el 2026-08-16: quedó desfasada porque las sesiones de captura escriben la tabla y nadie volvió a tocar el contexto. El número sale de la DB, no de esta tabla — verificar con `snapshot_counts.py` antes de citarlo |
| 2 | `inventory_weapons` | **56** | **Censo de rangos S y A CERRADO el 2026-09-12: 56 = el contador del header.** 46 equipadas (`origen_evidencia='s30_badge'`) + 10 libres (`'s30_libre'`, sin dueño; clave = arma + nivel + P + número de copia de la grilla). 2 filas con `descartado=1` son copias fantasma de un scroll (migs `_29`, `_31`). Rango B fuera de alcance. Las 50 de antes eran la transcripción de abril, vaciada en el rebuild del 2026-08-17 |
| 3 Thresholds | `agent_thresholds` | **114** | 48/52 PJs con ≥1 stat (Claret Flint: DEF/PV/CR de Prydwen) · Velina, Pyrois, Remielle Dan y **Aria** PENDIENTE (objetivos Prydwen, RNF-02) |
| 3 | `agent_score_thresholds` | **52** | Defaults equip 0.75 / stock 0.50, overridable (todos los 52 PJs) |
| 3 | `agent_substat_preferences` | **68** | 10 PJs con overrides (Claret Flint: 8, Prydwen); el resto cae al arquetipo del rol |
| 4 Scoring | `disc_archetypes` | **7** | ATK_DPS, HP_DISRUPT, ANOMALY, STUN, SUPPORT_ER, DEFENSE, **ARMORER_DEF** (mig 35). Mapeo rol→arquetipo: `repositories.ARCHETYPES_BY_ROLE`; un rol sin entrada avisa |
| 4 | `disc_set_archetype` | **45** | N:M con prioridad 1=primario, 2=secundario |
...
| 8 | `pj_weapon_synergy` | **300** | 50 PJs × 6 categorías (+ Claret Flint derivada del kit, ER sin fuente) (+ Pyrois matriz Ataque, + Remielle Dan y **Aria** matriz Anomalía) · Billy Estelar PENDIENTE (rol Disruptivos/Rupture sin matriz confirmada — RNF-02) |
...
**51 PJs** · Distribución elemento: Físico 14 · Eléctrico 12 · **Éter 9** · Fuego 9 · Hielo 5 · Viento 1 (Velina) · **Lumen 1 (Remielle Dan)**
...
**Distribución rol:** Ataque 15 · **Anomalía 11** · Aturdimiento 9 · Soporte 8 · Defensa 5 · Disruptivos 3
