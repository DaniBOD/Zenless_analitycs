...
| 1 | `agent_awakenings` | **15** | **Solo 1 con texto real** (Burnice nv6); las otras 14 son deuda. 4 `pending_capture` nv6 (Lycaon, Ellen, Grace, N.°0:Anby) + 5 placeholder "sin awakening" (Cissia, Billy Estelar, Velina, Pyrois, Remielle Dan) + **5 nuevas v3.1** (mig 16, 2026-07-29: N.°11, Harumasa, Nekomata, Rina, Jane) con **`nivel` en NULL a propósito** — el despertar existe pero no está confirmado si DaniBOD lo compró ni a qué nivel (RNF-02), y `activo=0` conservador hasta capturar. **9 filas en `pending_capture` en total.** El sistema de despertares no tiene captura implementada (cero refs a awakening/silueta en `app/core`): la carga es manual desde screenshots de la tienda Silueta Potencial ("Agotado"=nv6, "Límite ×N"=parcial) |
